return {
	...
	config = function()
		...
		local hl_target_win = function()
			...
			local possible_targets = vim.iter(vim.api.nvim_tabpage_list_wins(0))
				:filter(function(w)
			...
			local restore = function()
				vim.api.nvim_win_set_hl_ns(target_win_id, orig_hl_ns ~= -1 and orig_hl_ns or 0)
			end
		...
		local au_opts =
			{ pattern = "MiniFilesExplorerOpen", callback = hl_target_win, desc = "Highlight target window" }
